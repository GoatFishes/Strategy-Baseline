...
const testStats = async () => {
    try {
        ...
        utils.logEvent(constants.LOG_LEVELS.info, constants.RESPONSE_CODES.LOG_MESSAGE_ONLY, `Retrieve all orders the backtest performed`)
        const allOrders = await db.selectAllTrades()

        utils.logEvent(constants.LOG_LEVELS.info, constants.RESPONSE_CODES.LOG_MESSAGE_ONLY, `Loop through the orders to get the info`)
        ...
        utils.logEvent(constants.LOG_LEVELS.info, constants.RESPONSE_CODES.LOG_MESSAGE_ONLY, `Return the analised data`)
        setVars({ orderTimes, percentagePerformanceAverage, numberOfOrders })
    } catch (e) { throw new utils.ExceptionHandler(constants.RESPONSE_CODES.APPLICATION_ERROR, `Fatal error on backtest analysing : ${e}`) }
...
const avgTime = async function (params) {
    ...
    } catch (e) { throw new utils.ExceptionHandler(constants.RESPONSE_CODES.APPLICATION_ERROR, `Fatal error on average time analyser :  ${e}`) }
}
...
const avgReturn = async (params) => {
    try {
        ...
        if (contractZeroCounter === 0) {
            if (side === BUY) {
                ...
                assignedMargin += await utils.marginFormula({ orderQty, price, leverage })
            }
            else {
            ...
            if (side === BUY) {
                ...
                contractZeroCounter -= orderQty
                liberatedMargin += await utils.marginFormula({ orderQty, price, leverage })
            }
        }
    ...
    } catch (e) { throw new utils.ExceptionHandler(constants.RESPONSE_CODES.APPLICATION_ERROR, `Fatal error on average return analyser :  ${e}`) }
...
const setVars = async (params) => {
    try {
        ...
        await db.insertPerformance([avgTimePerOrder, averageReturn, overallPerformance, numberOfOrders, sharpeRatio, longestOrder, shortestOrder, bestOrder, worstOrder])
    } catch (e) { throw new utils.ExceptionHandler(constants.RESPONSE_CODES.APPLICATION_ERROR, `Fatal error backtest stats calculations : ${e}`) }
}
